...
class G_LOSS(nn.Module):
    def __init__(self, mode='lsgan'):
        ...
        self.criterionGAN = GANLoss(gan_mode=mode).to(device)
...
class AD_LOSS(nn.Module):
    def __init__(self, mode='lsgan'):
        ...
        # self.netD = NLayerDiscriminator(input_nc=1)
        device = torch.device('cuda:0')
        self.criterionGAN = GANLoss(gan_mode=mode).to(device)
...
if __name__ == "__main__":
    import numpy as np
    model = AD_LOSS()
    input_real = np.random.random((1, 1, 256, 256)).astype(np.float32)
    input_fake = np.random.random((1, 1, 256, 256)).astype(np.float32)

    input_real = torch.tensor(input_real)
    input_fake = torch.tensor(input_fake)
    out = model(input_real, input_fake)
