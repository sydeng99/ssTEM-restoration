...
def loop(cfg, train_provider, valid_provider, model, criterion, optimizer, iters, writer):
    ...
    if cfg.TRAIN.loss == 'L1':
        ...
        com_weight = cfg.TRAIN.com_weight
        if cfg.TRAIN.loss_combine == 'L1':
            criterion_combine = nn.L1Loss()
        else:
            criterion_combine = F.mse_loss
    else:
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        if cfg.TRAIN.loss == 'perceptual':
            loss_com = criterion_combine(pred, target)
            ...
            loss_vgg = vgg_loss(out1, out2, mode=1)
            loss = loss_com * com_weight + loss_vgg * vgg_weight
        elif cfg.TRAIN.loss == 'ssim':
            # pred = F.sigmoid(pred)
        ...
        if iters % cfg.TRAIN.display_freq == 0 or iters == 1:
            ...
            if cfg.TRAIN.loss == 'perceptual':
                writer.add_scalar('loss/loss_com', loss_com, iters)
