...
def loop(cfg, train_provider, valid_provider, model, criterion, optimizer, iters, writer):
    ...
    if cuda_count > 1:
        if cfg.TRAIN.batch_size % cuda_count == 0:
            print('FlowNet built on %d GPUs ... ' % cuda_count, flush=True)
            model_flow = nn.DataParallel(model_flow)
        else:
            raise AttributeError('Batch size (%d) cannot be equally divided by GPU number (%d)' % (cfg.TRAIN.batch_size, cuda_count))
    else:
        print('FlowNet built on a single GPU ... ', flush=True)
    model_path = cfg.TRAIN.flownet_path
    ckpt = cfg.TRAIN.flownet_id
    ...
    for k, v in state_dict.items():
        # name = k[7:] # remove module. # for for single GPU
        name = k   # for multiple GPUs
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        with torch.no_grad():
            pred_flow = model_flow(input)
        input_sff = input[:, :3].detach()
        # pdb.set_trace()
        pred_flow = pred_flow.permute(0,2,3,1)
        # pdb.set_trace()
        # warped_sff = F.grid_sample(input_sff, pred_flow, mode='bilinear', padding_mode='zeros')
        ...
        if iters % cfg.TRAIN.valid_freq == 0 or iters == 1:
            ...
            warped_sff = ((np.squeeze(warped_sff[0].data.cpu().numpy())) * 255).astype(np.uint8)
            warped_sff = np.transpose(warped_sff, (1,2,0))
            # input1 = input0[0:3]
            input1 = ((np.squeeze(input_sff[0].data.cpu().numpy())) * 255).astype(np.uint8)
            input2 = input0[3:6]
            ...
            pred_flow = np.squeeze(pred_flow[0].data.cpu().numpy())
            # pred_flow = np.transpose(pred_flow, (1,2,0))
            pred_show = dense_flow(pred_flow)
            pred = np.squeeze(pred[0].data.cpu().numpy())
            pred[pred>1] = 1; pred[pred<0] = 0
            pred = (pred * 255).astype(np.uint8)
            pred = pred[:, :, np.newaxis]
            pred = np.repeat(pred, 3, 2)
            im_cat1 = np.concatenate([input1, input2, target], axis=1)
            im_cat2 = np.concatenate([pred_show, warped_sff, pred], axis=1)
        ...
        if iters % cfg.TRAIN.save_freq == 0 or iters == 1:
            ...
            for k, data in enumerate(dataloader, 0):
                ...
                running_loss += loss
                if k == 0:
                    pred_flow = np.squeeze(pred_flow.data.cpu().numpy())
                    # pred_flow = np.transpose(pred_flow, (1,2,0))
                    ...
                    sff = ((np.squeeze(input_sff[0].data.cpu().numpy())) * 255).astype(np.uint8)
                    sff = np.transpose(sff, (1,2,0))
                    im_cat1 = np.concatenate([sff, flow_show], axis=1)
                    im_cat2 = np.concatenate([pred, gt], axis=1)
                    im_cat = np.concatenate([im_cat1, im_cat2], axis=0)
